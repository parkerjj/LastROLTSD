...
// @version      1.0.0
// @description  让露天商店.Ro「菜农监控台」的查询请求由浏览器直接发送给 LastRO 官方服务器，账号密码不再经过本站中转。
// @author       LastROLTSD
  ...
  function queryOfficial(userid, userPass) {
    return new Promise(function (resolve, reject) {
      GM_xmlhttpRequest({
        ...
        data:
          "Login_debug%5Buserid%5D=" +
          encodeURIComponent(userid) +
          "&Login_debug%5Buser_pass%5D=" +
        ...
        onload: function (res) {
          ...
          if (payload === 1) resolve({ state: "auth_failed" });
            ...
            resolve({ state: "online", data: payload });
          else reject(new Error("LastRO 官方接口返回异常，请稍后再试"));
        },
        onerror: function () {
          reject(new Error("无法连接 LastRO 官方服务器，请稍后再试"));
        },
        ontimeout: function () {
          reject(new Error("连接 LastRO 官方服务器超时，请稍后再试"));
        },
      });
    });
  }
