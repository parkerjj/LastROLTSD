...
type RenderState = Pick<SearchControllerState, 'loading' | 'error' | 'empty' | 'pageIndex'> & {
  cursor?: string | null;
...
function statePanelMarkup(kind: 'empty' | 'error', title: string, description: string, actionLabel?: string): string {
...
const GROUPS = [
  ...
  { key: 'shop', label: '商店名称命中', hint: '商店标题中包含搜索词 · 按商店聚合' },
  { key: 'vendor', label: '商人名称命中', hint: '摆摊玩家名称中包含搜索词 · 按玩家聚合' },
] as const;
...
export function renderSearchResults(container: HTMLElement, page: SearchPage<ListingSearchResult>, state: RenderState): void {
  ...
  if (state.loading && safeItems.length === 0) { container.innerHTML = '<p role="status" class="state-message">正在加载市场数据...</p>'; return; }
  if (state.error) {
    container.innerHTML = statePanelMarkup('error', '市场查询失败，请稍后再试', friendlyError(state.error, '市场查询接口暂不可用，请确认本地服务已启动。'), '重新加载');
    return;
  }
  ...
  const groups = groupListings(safeItems, query);
  ...
  const resultCountLabel = state.initialBrowse ? `最新收录 ${rangeLabel} 个商品` : `查看 ${rangeLabel} 个商品`;
  ...
  const bottomPager = pagerBarMarkup(page, state, pageIndex, rangeLabel, 'is-bottom');
...
function pagerBarMarkup(page: SearchPage<ListingSearchResult>, state: RenderState, pageIndex: number, rangeLabel: string, extraClass: string): string {
...
function renderItemMarketHistory(drawer: HTMLElement, history: ItemMarketHistory, item?: HistoryItemBrief): void {
  ...
  const earliestEvent = history.events.reduce((min, event) => Math.min(min, event.observedAt), history.windowEnd);
  const eventRange = formatDateRange(earliestEvent, history.windowEnd);
  const trendKicker = history.events.length === 0 ? '市场走势' : `${describeTimeSpan(history.windowEnd - earliestEvent)}市场走势`;
...
function renderHistoryTrend(history: ItemMarketHistory): string {
  ...
  if (events.length === 0) return '<p class="history-empty">暂时没有足够的价格或库存变动，无法绘制走势。</p>';
  ...
  const priced = samples.filter((sample): sample is TrendSample & { min: number; median: number; max: number } => sample.median !== null);
  const frame = TREND_FRAME;
  const span = Math.max(1, chartEnd - chartStart);
  const xOf = (value: number) => frame.left + ((value - chartStart) / span) * frame.width;
  ...
  if (priced.length > 0) {
    ...
    const padding = Math.max(1, (hi - lo) * .06);
    priceLo = Math.max(0, lo - padding);
    priceHi = hi + padding;
  }
  const stockHi = Math.max(1, ...samples.map((sample) => sample.stock)) * 1.18;
  const yPrice = (value: number) => frame.top + (1 - (value - priceLo) / (priceHi - priceLo)) * frame.height;
  const yStock = (value: number) => frame.top + (1 - value / stockHi) * frame.height;
  const rounded = (value: number) => Math.round(value * 100) / 100;

  // Horizontal grid rows with both-axis labels.
  const gridRows = [0, .25, .5, .75, 1].map((fraction) => {
    const y = frame.top + (1 - fraction) * frame.height;
    const line = fraction > 0 && fraction < 1 ? `<line x1="${frame.left}" y1="${y}" x2="${frame.left + frame.width}" y2="${y}" class="history-chart-grid"/>` : '';
  ...
  for (let start = 0; start < samples.length; ) {
    if (samples[start]!.median === null) { start += 1; continue; }
    let end = start;
    while (end < samples.length && samples[end]!.median !== null) end += 1;
    const run = samples.slice(start, end);
    const upper = run.map((sample) => `L${rounded(xOf(sample.time))} ${rounded(yPrice(sample.max!))}`).join('');
    const lower = run.slice().reverse().map((sample) => `L${rounded(xOf(sample.time))} ${rounded(yPrice(sample.min!))}`).join('');
  ...
  if (firstKnown !== -1) {
    ...
    let top = `M${x(0)} ${y(0)}`;
    for (let index = 1; index < run.length; index += 1) top += `H${x(index)}L${x(index)} ${y(index)}`;
  ...
  const rawDots = events.length <= 20
    ? events.filter((event) => event.quantity > 0).map((event) => `<circle cx="${rounded(xOf(event.observedAt))}" cy="${rounded(yPrice(event.price))}" r="2.2" class="history-chart-rawpoint"><title>${formatTipDate(event.observedAt)} · ${event.price.toLocaleString('zh-CN')} z</title></circle>`).join('')
    : '';

  // X 轴刻度按跨度动态选择（整点小时 / 自然日），跨日 0 点显示月日。
  const dateTicks = buildTimeTicks(chartStart, chartEnd).map((tick, index, ticks) => {
...
function buildTrendSamples(events: ItemMarketHistory['events'], chartStart: number, chartEnd: number): TrendSample[] {
  const span = Math.max(1, chartEnd - chartStart);
  const states = new Map<number, { price: number; quantity: number; active: boolean }>();
  let cursor = 0;
  ...
  for (let index = 0; index < TREND_SAMPLE_COUNT; index += 1) {
    ...
    while (cursor < events.length && events[cursor]!.observedAt <= time) {
      ...
      states.set(event.listingId, {
        ...
        quantity: typeof event.quantity === 'number' ? event.quantity : previous?.quantity || 0,
        active: !INACTIVE_EVENT_TYPES.has(event.eventType),
      });
      cursor += 1;
    }
    if (states.size === 0) {
      samples.push({ time, min: null, median: null, max: null, stock: 0, shops: 0, known: false });
      continue;
    }
    const prices: number[] = [];
    let stock = 0;
    ...
    states.forEach((state) => {
      ...
      if (state.quantity > 0) { prices.push(state.price); shops += 1; }
    });
    if (prices.length === 0) {
      samples.push({ time, min: null, median: null, max: null, stock, shops, known: true });
      continue;
    }
    prices.sort((a, b) => a - b);
    ...
    samples.push({
      time,
      min: prices[0]!,
      median: prices.length % 2 === 0 ? (prices[middle - 1]! + prices[middle]!) / 2 : prices[Math.floor(middle)]!,
      max: prices.at(-1)!,
      stock,
      shops,
      known: true,
...
function attachHistoryChart(drawer: HTMLElement): void {
  ...
  try {
    points = JSON.parse(decodeURIComponent(svg.dataset.trend ?? '')) as TrendPointView[];
  } catch {
    return;
  }
  ...
  const show = (index: number): void => {
    ...
    else { priceDot.setAttribute('visibility', 'visible'); priceDot.setAttribute('cy', String(point.ym)); }
    hover.setAttribute('visibility', 'visible');

    tip.innerHTML = `<p class="history-chart-tip-date">${formatTipDate(point.t)}</p>${point.med === null
      ? '<p class="history-chart-tip-row"><span>报价</span><b>该时段无在售</b></p>'
      : `<p class="history-chart-tip-row"><span>中位价</span><b>${point.med.toLocaleString('zh-CN')} z</b></p><p class="history-chart-tip-row"><span>区间</span><b>${point.lo!.toLocaleString('zh-CN')} – ${point.hi!.toLocaleString('zh-CN')} z</b></p>`}<p class="history-chart-tip-row"><span>库存</span><b>${point.stock.toLocaleString('zh-CN')} 件 · ${point.shops} 间</b></p>`;
    ...
    tip.classList.toggle('is-left', point.x > TREND_VIEW.width * .55);
  ...
  svg.addEventListener('keydown', (event) => {
    ...
    else if (event.key === 'End') { event.preventDefault(); show(points.length - 1); }
...
function buildTimeTicks(start: number, end: number): Array<{ time: number; label: string }> {
  const span = Math.max(1, end - start);
  // 目标 4–6 个刻度：取第一个让 span/step <= 6 的自然步长。
  const spec = TICK_STEPS.find((candidate) => span / candidate.step <= 6) ?? TICK_STEPS.at(-1)!;
  ...
  if (ticks.length === 0 || ticks[0]!.time - start > span * .18) {
    ticks.unshift({ time: start, label: formatEdgeTickLabel(start, spec) });
  }
  if (end - ticks[ticks.length - 1]!.time > span * .18) {
...
function floorTickBoundary(value: number, spec: TimeTickSpec): number {
  ...
  return new Date(aligned.getUTCFullYear(), aligned.getUTCMonth(), aligned.getUTCDate()).getTime();
}
...
function formatTickLabel(value: number, spec: TimeTickSpec): string {
  ...
  if (spec.byHour) {
    ...
    if (date.getHours() === 0) return `${date.getMonth() + 1}/${date.getDate()}`;
...
function aggregateItems(items: ListingSearchResult[], keyOf: (item: ListingSearchResult) => string): Aggregate[] {
...
function groupCountLabel(key: string, items: ListingSearchResult[]): string {
  ...
  if (key === 'shop') return `${aggregateItems(items, storeKey).length} 间商店 · ${items.length} 件商品`;
  return `${aggregateItems(items, (item) => String(item.vendorName ?? '')).length} 位商人 · ${items.length} 件商品`;
}

function groupBody(key: string, items: ListingSearchResult[], query?: string): string {
  if (key === 'name') return `<div class="name-stack">${items.map((item) => renderMarketCard(item, query)).join('')}</div>`;
...
function optionText(item: ListingSearchResult): string {
  const text = item.options.map((option) => option.display || `未知词条 type=${option.type} value=${option.value} param=${option.param}`).join('、');
  return text || '暂无词条';
}
...
function itemIconMarkup(item: ListingSearchResult): string {
  ...
  if (/[<>]/u.test(rawName)) return `<span class="item-icon-fallback" aria-hidden="true">${escape(item.itemId)}</span>`;
...
function renderMarketCard(item: ListingSearchResult, query?: string): string {
  ...
  return `<article class="market-card">
    ...
    <div class="mc-main">
      ...
      <p class="mc-options">${escape(optionText(item))}</p>
    </div>
    <div class="mc-price">
    ...
    <div class="mc-actions">
      ...
      ${relativeTimeMarkup(item.lastChangedAt, 'mc-time')}
    </div>
  </article>`;
}
...
function renderCompactLine(item: ListingSearchResult): string {
  ...
  return `<li class="line">
    ...
    <span class="line-name"><strong>${escape(itemName)}</strong><small>${escape(optionText(item))}</small></span>
...
function renderShopCard(shop: Aggregate, query?: string): string {
  ...
  const latest = Math.max(...shop.items.map((item) => Number(item.lastChangedAt)));
...
function renderVendorGroup(items: ListingSearchResult[], query?: string): string {
  const vendors = aggregateItems(items, (item) => String(item.vendorName ?? ''));
...
function renderVendorCard(vendor: Aggregate, query?: string): string {
  ...
  const latest = Math.max(...vendor.items.map((item) => Number(item.lastChangedAt)));
...
function highlight(value: unknown, query?: string): string {
  ...
  return parts.map((part, index) => (index % 2 === 1 ? `<mark>${escape(part)}</mark>` : escape(part))).join('');
}
...
function startRelativeTimer(): void {
  if (relativeTimer !== undefined || typeof document === 'undefined' || typeof window === 'undefined') return;
  relativeTimer = window.setInterval(() => refreshRelativeTimes(document), 30_000);
}
