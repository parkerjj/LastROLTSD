...
results.addEventListener('click', (event) => {
  ...
  if (historyButton) {
    ...
    if (Number.isSafeInteger(itemId)) void openHistory({ itemId, ...(itemName === undefined ? {} : { itemName }), ...(itemIcon === undefined ? {} : { itemIcon }) });
    return;
  }
...
results.addEventListener('change', (event) => {
  ...
  if (!(target instanceof HTMLSelectElement) || target.id !== 'result-sort') return;
