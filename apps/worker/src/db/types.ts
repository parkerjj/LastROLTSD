...
export interface ListingRow { id: number; shopSessionId: number; itemFingerprint: string; itemKey: string | null; itemId: number; upgrade: number; slots: number; cards: number[]; price: number; quantity: number; lastQuantity: number; status: string; stateVersion: number; missingStreak: number; lastSeenAt: number; }
...
export interface ListingSearchRow extends ListingRow { itemName: string; shopKey: string; title: string; vendorName: string; mapName: string; shopType: 'buy' | 'sell'; options: ListingOption[]; }
