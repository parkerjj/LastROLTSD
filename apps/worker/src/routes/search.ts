...
export function registerSearchRoute(app: Hono<any>, repo: MarketRepository, cursorSecret?: string): void {
  app.get('/api/v1/market/search', (c) => searchResponse(c.req.raw, repo, cursorSecret));
}

export async function searchResponse(request: Request, repo: MarketRepository, cursorSecret?: string, verifyCursor = true): Promise<Response> {
  try {
    const [catalogVersion, definitions] = await Promise.all([repo.getCatalogVersion(), repo.getOptionDefinitions()]);
    const filters = parseSearchParams(new URL(request.url), { ...(cursorSecret === undefined ? {} : { cursorSecret }), verifyCursor, catalogVersion, optionVersion: definitions.version, searchIndexVersion: SEARCH_INDEX_VERSION });
