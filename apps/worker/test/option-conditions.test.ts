...
const definitions: OptionDefinitionMap = new Map<number, OptionDefinition>([
  [12, { type: 12, handle: 'atk_plus', labelZh: 'ATK +', descriptionTemplate: '攻击力增加 {value}', valueType: 'integer', unit: 'points', scale: 1, allowedOperators: ['eq', 'neq', 'gt', 'gte', 'lt', 'lte'], paramPolicy: { mode: 'ignored', filterable: false }, repeatPolicy: 'same', displayTemplate: 'ATK + {value}' }],
  [20, { type: 20, handle: 'rate', labelZh: '倍率', descriptionTemplate: '倍率 {value}', valueType: 'scaled_integer', unit: '%', scale: 100, allowedOperators: ['eq', 'gte'], paramPolicy: { mode: 'required_exact', value: 7, filterable: true }, repeatPolicy: 'distinct', displayTemplate: '倍率 {value}' }],
]);
...
describe('option conditions', () => {
  ...
  it('rejects unknown and definition-disallowed operators', () => {
    ...
    expect(() => parseOptionCondition('20:neq:1', definitions)).toThrow('operator is not allowed');
  ...
  it('parses scaled decimal values as exact integers and rejects exponent or excess precision', () => {
    expect(parseOptionCondition('20:gte:1.50:7', definitions)).toMatchObject({ rawValue: 150, displayValue: '1.50', param: 7 });
    expect(() => parseOptionCondition('20:gte:1e2:7', definitions)).toThrow('Invalid option value');
    expect(() => parseOptionCondition('20:gte:1.234:7', definitions)).toThrow('Invalid option value');
    expect(() => parseOptionCondition('20:gte:1.500:7', definitions)).toThrow('Invalid option value');
  });

  it('enforces required and forbidden params', () => {
    expect(() => parseOptionCondition('20:gte:1.5', definitions)).toThrow('param is required');
  ...
  it('uses same occurrence semantics for repeated types and distinct occurrence semantics when declared', () => {
    ...
    const distinct = compileOptionPredicates([
      parseOptionCondition('20:gte:1.5:7', definitions), parseOptionCondition('20:eq:2:7', definitions),
    ], 'all', definitions);
  ...
  it('renders display text from definitions and raw tuples only', () => {
    expect(formatOptionDisplay({ type: 12, value: 50, param: 0 }, definitions.get(12))).toBe('ATK + 50');
    expect(formatOptionDisplay({ type: 20, value: 150, param: 7 }, definitions.get(20))).toBe('倍率 1.50');
